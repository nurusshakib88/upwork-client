...
const OrderTable = ({ storeId, tDate, clientData }) => {
  const [data, setData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("/api/api/Home/HeaderTrans", {
          params: {
            logedstoreid: storeId,
            logedclientid: clientData?.id || null,
            logedseldate: tDate,
          },
          headers: {
        ...
        console.error("Error fetching the order table data", error);
        setData([]);
      }
    };

    if (storeId && tDate && clientData) {
      fetchData();
    }
  }, [storeId, tDate, clientData]);
